...
- `backend/` — Fastify + TypeScript, SQLite (`better-sqlite3` + Drizzle ORM), Server-Sent Events for live updates
...
- Live event stream: `curl -N http://localhost:3001/api/events`
...
records), the code4rena/translate parsing and batching logic, the SSE connection cap, and the `/api/programs`
routes (including the closed-programs default filter) via Fastify's `inject()`, all against a disposable temp-file
database created fresh per test file. Frontend tests cover the programs store's filtering/sorting logic with the
API layer mocked. CI (`.github/workflows/ci.yml`) runs lint, format check, typecheck, tests, the production build,
...
- Frontend (nginx, serves the built SPA and proxies `/api/*` and the `/api/events` SSE stream to the backend): http://localhost
